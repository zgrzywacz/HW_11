...
for (year in 1985:2019){
  ...
  tempdata <- ncdc(datasetid='GSOM', stationid = 'GHCND:ASN00091245', datatypeid='TAVG', startdate = Start, enddate = End, limit = 100, includemetadata = FALSE)
...
```{r}
SumTemp <- setNames(data.frame(matrix(ncol = 3, nrow = 35)), c("Year", "Tavg", "Mons"))
rowsToDelete <- c()
for(x in 1:35){
  currentdata <- data.frame(justdata[x])
  if(nrow(currentdata) >= 1){
    SumTemp[x,1] <- (1984 + x)
    SumTemp[x,2] <- (colSums(currentdata[4])/nrow(currentdata))
    SumTemp[x,3] <- nrow(currentdata)
  }
  
SumTemp <- na.omit(SumTemp)
}
```
